...
def test_vsa_trtllm_uses_generic_lifecycle_without_forward_override() -> None:
    ...
    assert "sparse_predict" in VSATrtllmAttention.__dict__
...
def test_vsa_predictor_produces_sorted_block_inputs_and_effective_tiled_qkv() -> None:
    ...
    assert isinstance(inputs.block_sparse_inputs, BlockSparseForwardInputs)
    assert inputs.block_sparse_inputs.block_indptr.tolist() == [[[0, 2, 4]]]
    assert inputs.block_sparse_inputs.block_indices.tolist() == [0, 1, 0, 1]
    assert inputs.block_sparse_inputs.kv_valid_bits.dtype == torch.uint32
    assert inputs.block_sparse_inputs.kv_valid_bits.tolist() == [
...
def test_trtllm_vsa_dense_fallback_predicts_compact_inputs() -> None:
    ...
    with set_vsa_forward_context(_make_vsa_metadata(sparsity=0.5)):
        inputs = attention.sparse_predict(
            q,
            q,
            q,
            batch_size=1,
...
def test_trtllm_vsa_rejects_caller_block_inputs() -> None:
    attention = object.__new__(VSATrtllmAttention)
    q = torch.randn(1, 64, 1, 8)
    caller_inputs = BlockSparseForwardInputs(
        q_block_size=64,
        kv_block_size=64,
    ...
    with pytest.raises(ValueError, match="must be produced by sparse_predict"):
        attention.forward(
            ...
            batch_size=1,
            seq_len=64,
            block_sparse_inputs=caller_inputs,
        )
