...
def test_vsa_sparse_backends_match_on_ragged_input():
    ...
    for backend, setup in setups.items():
        if backend == "CUTEDSL":
            ...
            original_predict = setup.integrated.attn.sparse_predict

            def checked_predict(*args, _original=original_predict, **kwargs):
                result = _original(*args, **kwargs)
                sparse_fine_executed["TRTLLM"] = result.block_sparse_inputs is not None
                return result

            setup.integrated.attn.sparse_predict = checked_predict
