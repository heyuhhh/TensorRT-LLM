...
def test_sol_backend_warmup_prepares_phase_for_capture(monkeypatch) -> None:
    ...
    assert _forward(backend, q, q, q, timestep=torch.tensor(0.8)) is q
    monkeypatch.setattr(torch.cuda, "is_current_stream_capturing", lambda: True)
    assert _forward(backend, q, q, q, timestep=torch.tensor(0.8)) is q
...
def test_sol_backend_rejects_cutoff_capture_without_warmup(monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="prepared before CUDA Graph capture"):
        _forward(backend, q, q, q, timestep=torch.tensor(0.2))
...
@_CPU_ONLY
def test_sol_explicit_phase_overrides_timestep_without_model_scope(monkeypatch) -> None:
    q = _bshd()
    params = SolParams(tau=1.0, disabled_until_timestep=0.6)
    backend, predictor = _stub_backend(params)
    predictor.predict.return_value = _predictor_outputs(batch_size=1, seq_len=64, num_heads=2)
    monkeypatch.setattr(TrtllmAttention, "_forward_impl", lambda self, *args, **kwargs: q)
    monkeypatch.setattr(sol_backend, "get_bmm1_scale", lambda attn: 0.125)

    assert _forward(backend, q, q, q, timestep=0.8, sol_graph_phase=1) is q
...
def test_sol_backend_sparse_phase_emits_proxy_bitmask_carrier(monkeypatch) -> None:
    ...
    runtime_params = forwarded["sparse_runtime_params"]
    carrier = runtime_params.block_sparse_inputs
...
def test_sol_cuda_graph_phase_is_keyed_without_model_scope(monkeypatch) -> None:
    ...
    assert model(q, timestep=torch.tensor(0.8)) is q
