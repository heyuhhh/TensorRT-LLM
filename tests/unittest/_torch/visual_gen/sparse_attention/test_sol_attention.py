...
def test_sol_config_lowers_and_factory_initializes_backend(monkeypatch) -> None:
    ...
    assert backend._enable_sparse_workflow
...
def test_sol_backend_sparse_phase_emits_proxy_bitmask_carrier(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        PrimsTSFmha,
        "_get_bmm1_scale",
        staticmethod(lambda attn: 0.375),
    )
    ...
    carrier = forwarded["block_sparse_inputs"]
    assert (
...
def test_sol_cuda_graph_phase_is_keyed_and_scoped_during_capture(monkeypatch) -> None:
    ...
    monkeypatch.setattr(PrimsTSFmha, "_get_bmm1_scale", staticmethod(lambda attn: 0.125))
    capturing = False
    captured_outputs = {}
    captured_keys = []
