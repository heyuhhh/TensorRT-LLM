...
def test_batch_sparse_kv_predict(batch_size, num_contexts):
    """
    Test batch_sparse_kv_predict against vanilla _get_snapkv_indices.
    ...
    trtllm_sparse_kv_indices, trtllm_sparse_kv_offsets = trtllm_attn.batch_sparse_kv_predict(
        qkv, trtllm_metadata)
...
def test_batch_sparse_attn_predict(batch_size, num_contexts):
    """Test batch_sparse_attn_predict against vanilla _rocketkv_selection."""
    ...
    trtllm_sparse_attn_indices, trtllm_sparse_attn_offsets = trtllm_attn.batch_sparse_attn_predict(
        qkv, trtllm_metadata)
...
if __name__ == '__main__':
    ...
    # Unit tests for batch_sparse_kv_predict
    print("\n=== Testing batch_sparse_kv_predict ===")
    test_batch_sparse_kv_predict(1, 1)  # bs=1, context only
    test_batch_sparse_kv_predict(2, 2)  # bs=2, context only
    test_batch_sparse_kv_predict(6, 3)  # bs=6, mixed (3 ctx + 3 gen)

    # Unit tests for batch_sparse_attn_predict
    print("\n=== Testing batch_sparse_attn_predict ===")
    test_batch_sparse_attn_predict(1, 0)  # bs=1, generation only
    test_batch_sparse_attn_predict(2, 0)  # bs=2, generation only
    test_batch_sparse_attn_predict(5, 3)  # bs=5, mixed (3 ctx + 2 gen)
