...
def test_block_sparse_wrapper_cache_is_shared_by_compatible_attention_layers(
    monkeypatch,
) -> None:
    ...
    for _ in range(40):
        attention = _Attention()
        attention._block_sparse_fmha_cache_state = shared_cache_state
        attentions.append(attention)
        fmhas.append(fmha_module.PrimsTSBlockSparseFmha(attention))

    assert all(fmha._contiguous_wrappers is fmhas[0]._contiguous_wrappers for fmha in fmhas)
    assert all(fmha._paged_wrappers is fmhas[0]._paged_wrappers for fmha in fmhas)

    wrapper = Mock()
    factory = Mock(return_value=wrapper)
    monkeypatch.setattr(fmha_module, "_BlockSparseTSWrapper", factory)
    inputs = _bitmask_inputs(proxy=True)
    q = torch.empty((1, 128, 2, 128), dtype=torch.bfloat16)
    keys = [
        fmha._make_plan_key(
            q,
            inputs,
    ...
    planned = [fmha._get_or_plan_wrapper(key, paged=False) for fmha, key in zip(fmhas, keys)]

    assert all(current is wrapper for current in planned)
    assert factory.call_count == 1
    assert wrapper.plan.call_count == 1


def test_shared_block_sparse_plan_key_includes_attention_head_topology() -> None:
    _, fmha_module = _generic_api()
    inputs = _bitmask_inputs(proxy=True)
...
def test_paged_wrapper_is_planned_once_with_all_metadata_live(monkeypatch) -> None:
    ...
    torch.testing.assert_close(snapshots[0][1], torch.arange(8, dtype=torch.int32))
    torch.testing.assert_close(snapshots[1][1], torch.arange(8, dtype=torch.int32) + 10)
    assert snapshots[0][2] is first_inputs.block_indptr
    assert snapshots[1][2] is args.block_sparse_inputs.block_indptr
...
def test_prepare_workspace_checks_capture_before_resize(monkeypatch) -> None:
    ...
    monkeypatch.setattr(fmha, "_ensure_metadata_buffers", Mock())
    fmha._multi_processor_count = 1
