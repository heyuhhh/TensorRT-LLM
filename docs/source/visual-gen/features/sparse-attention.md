...
Sparse attention in VisualGen is configured through `VisualGenArgs.attention_config.sparse_attention_config`. The user-facing config stays in VisualGen args or model config, while `attention_config.backend` selects the kernel family. VisualGen organizes concrete sparse attention backends under `visual_gen/attention_backend/sparse/`, mirroring the LLM sparse tree. Shared predictors produce per-call inputs such as `BlockSparseForwardInputs`; backend classes then execute those inputs through their normal attention path.
...
- `disabled_until_timestep` — optional normalized `[0, 1]` transformer-forward timestep cutoff. Denoising starts near 1 and moves toward 0, so Skip Softmax Attention is disabled while `timestep >= disabled_until_timestep` and enabled after the timestep drops below the cutoff. Every transformer forward must provide `timestep` when this cutoff is configured. For per-token timesteps, VisualGen uses the maximum value, keeping the whole attention call dense until every token has crossed the cutoff.
...
`PrimsTSBlockSparseFmha` library then executes that route through
`BlockSparseForwardInputs`. `SOLTrtllmAttention` is only the VisualGen bridge;
...
with 4-D BSHD Q/K/V tensors, equal Q/K/V shapes, and head dimension 128. Fused
projection split views are compacted once before prediction. Cross-attention,
context parallelism, attention quantization, and unsupported tensor envelopes
raise an error instead of silently falling back to dense attention.
...
When a cutoff is configured, VisualGen resolves the dense-or-sparse phase once
per model forward and includes it in the CUDA Graph key. This avoids a device
scalar read in every attention layer, keeps predictor route buffers stable for
replay, and prevents a dense capture from being reused for the sparse phase.
...
VSA belongs entirely to VisualGen and lives under `visual_gen/attention_backend/sparse/vsa/`. It defines concrete TRTLLM and CuTe DSL attention backends while remaining separate from the core FMHA registry:

- `metadata.py` owns VSA shape, policy, and forward-context metadata.
- `predictor.py` owns the single `VSAPredictor`, including tiling, coarse attention,
  route selection, and typed effective Q/K/V plus optional
  `BlockSparseForwardInputs` and post-process context.
- `backend.py` owns backend-specific QKV normalization and execution. It provides
  `VSATrtllmAttention`, which consumes predictions through the normal TRTLLM
  lifecycle, and `VSACuTeDSLAttention`, which reuses the same predictor and
  post-processing while retaining only CuTe DSL-specific fine execution.
