...
class SOLTrtllmAttention(TrtllmAttention[SparseForwardInputs]):
    ...
    def _should_use_sparse_workflow(self, forward_kwargs: dict[str, object]) -> bool:
        """Consume the per-call phase and select dense or SOL execution."""

        explicit_phase = forward_kwargs.pop("sol_graph_phase", None)
        scoped_phase = self.sol_params.get_scoped_graph_phase()
        if scoped_phase is not None and explicit_phase not in (None, scoped_phase):
            raise ValueError("sol_graph_phase conflicts with the active model-forward phase")
        graph_phase = scoped_phase if scoped_phase is not None else explicit_phase
    ...
    def sparse_predict(
        ...
        *,
        ...
        """Build effective compact QKV and explicit block-sparse inputs."""
        ...
        outputs = self.predictor.predict(
            ...
            sm_scale=PrimsTSFmha._get_bmm1_scale(self),
        )
        ...
        remaining_kwargs.pop("sol_graph_phase", None)
