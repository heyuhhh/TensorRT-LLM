...
from .common import VSA_BLOCK_SIZE, VSAMetadata, _normalize_qkv_inputs
...
class VSATrtllmAttention(TrtllmAttention[VSAForwardInputs]):
    ...
    def sparse_predict(
        ...
        *,
        ...
        use_primts = any(isinstance(fmha, PrimsTSBlockSparseFmha) for fmha in self.fmha_libs)
        ...
        if not use_primts:
            logger.warning_once(
                ...
                "prims_ts_block_sparse FMHA library is disabled; using the compact "
    ...
    def sparse_post_process(
        self,
        output: torch.Tensor,
        sparse_inputs: VSAForwardInputs,
    ) -> torch.Tensor:
