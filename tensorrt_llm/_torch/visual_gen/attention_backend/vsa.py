...
"""
Shared Video Sparse Attention (VSA) algorithm for visual generation models.

VSAAttentionBase implements hierarchical sparse attention:
  - Coarse branch: mean-pooled cube attention (always dense)
  - Fine branch: a backend-specific block-sparse top-K implementation, with
    dense SDPA as the common unsupported-envelope fallback.
"""
...
# A 4x4x4 cube is one 64-token sparse block for both fine-stage backends.
...
class VSAMetadataBuilder:
    ...
    def _build_shape_payload(
        ...
        kv_token_mask = (tile_partition_indices >= 0).contiguous()
...
class VSAAttentionBase(AttentionBackend):
    """
    Video Sparse Attention (VSA) backend for diffusion models.

    Implements coarse mean-pool + fine block-sparse top-K attention.
    Subclasses provide the fine-stage block-sparse kernel. Returning ``None``
    from that hook selects the common dense SDPA fallback.
    ...
    def _forward_sparse_fine(
        ...
        """Run a backend-specific fine stage, or return ``None`` for dense fallback."""
    ...
    def forward(
        ...
        **kwargs,
        ...
        o_f_tiled = self._forward_sparse_fine(
            q_t,
            k_t,
            v_t,
            topk_indices,
