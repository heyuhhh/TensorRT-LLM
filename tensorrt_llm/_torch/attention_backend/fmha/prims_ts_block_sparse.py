...
class PrimsTSBlockSparseFmha(PrimsTSFmha):
    def __init__(self, attn: "TrtllmAttention") -> None:
        super().__init__(attn)
        shared_cache_state = getattr(attn, "_block_sparse_fmha_cache_state", None)
        if shared_cache_state is None:
            shared_cache_state = {
                "contiguous_wrappers": {},
                "paged_wrappers": {},
            }
        self._contiguous_wrappers = cast(
            ...
            shared_cache_state.setdefault("contiguous_wrappers", {}),
        )
        self._paged_wrappers = cast(
            dict[_BlockSparsePlanKey, "BlockSparsePagedTSWrapper"],
            shared_cache_state.setdefault("paged_wrappers", {}),
        )
    ...
    def prepare_workspace(
        ...
        if block_offsets is None:
            raise RuntimeError("PrimTS block-sparse attention requires paged KV-cache offsets")
        with torch.cuda.device(q.device):
            self._ensure_metadata_buffers(
                q.device,
                max(int(metadata.max_num_requests), 1),
                int(block_offsets.shape[-1]),
    ...
    def _contiguous_unsupported_reason(
        ...
        try:
            q_view, _, _, _ = self._contiguous_views(
                q,
                k,
                v,
                route_tensor,
